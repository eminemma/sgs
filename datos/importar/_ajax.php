...
switch ($accion) {
    ...
    case 'importar_loteria_anticipada':
        ...
        try {
											...
											WHERE R.SUC_BAN          = A.SUC_BAN(+)
											  ...
											  AND (R.SUC_BAN_DEVUELVE IS NULL
											   ...
											   AND OCR IS NOT NULL
												...
												"
                , array($id_juego, $sorteo, $id_juego, $sorteo));
            ...
            sql("DELETE FROM SGS.T_ANTICIPADA WHERE SORTEO = ? AND ID_JUEGO = ?", array($sorteo, $id_juego));

            $rs_anticipado = sql_kanban("SELECT	'INSERT
											INTO SGS.T_ANTICIPADA
											  (
											    ...
											    IMPORTE,
                          						ORDEN
											    ) VALUES ('||TPA.ID_JUEGO||','||TPA.SORTEO||','|| TPA.SEMANA||','''||CONVERT(TPA.DESCRIPCION, 'WE8ISO8859P1', 'AL32UTF8')||''','||DECODE(TS.FECHA_HASTA_PAGO_PREMIO,NULL,'NULL',''''||TO_CHAR(TS.FECHA_HASTA_PAGO_PREMIO,'DD/MM/YYYY'))||''','||DECODE(TPA.FECHA_PROXIMO_SORTEO,NULL,'''NULL',''''||TO_CHAR(TPA.FECHA_PROXIMO_SORTEO,'DD/MM/YYYY'))||''','''||  (
												  SELECT CONVERT(TPP.DESCRIPCION, 'WE8ISO8859P1', 'AL32UTF8')
												  FROM KANBAN.T_PROGRAMA_PREMIOS_ANTIC TPP
												  WHERE TPP.ID_PROGRAMA_PREMIOS_ANTIC > TPA.ID_PROGRAMA_PREMIOS_ANTIC
												    ...
												    AND ROWNUM                        = 1
												  )||''','||TPA.IMPORTE||','||TPA.ORDEN||')' AS EXEC

											FROM  KANBAN.T_PROGRAMA_PREMIOS_ANTIC TPA,KANBAN.T_SORTEO TS
											...
											ORDER BY TPA.SEMANA ASC", array($sorteo, $id_juego));

            //$db->debug = true;
            ComenzarTransaccion($db);
            while ($row_anticipado = siguiente_kanban($rs_anticipado)) {
                sql($row_anticipado->EXEC);
            }

            FinalizarTransaccion($db);
            $sql_migrar_anticipado = "	SELECT 'INSERT
											INTO SGS.T_BILLETES_PARTICIPANTES
											  (
												ID_JUEGO,
												SORTEO,
							  				...
							  				VALUES
							  				('||ID_JUEGO||','||SORTEO||','||BILLETE||','||DECODE(SUC_BAN,NULL,'NULL',SUC_BAN)||','||DECODE(NRO_AGEN,NULL,'NULL',NRO_AGEN)||','''||DESCRIPCION_AGENCIA||''','''||LOCALIDAD||''','''||PROVINCIA||''','||FRACCION||','||PROGRESION||','||SERIE||','''||DESCRIPCION_SUCURSAL||''','''||MODALIDAD||''')' AS EXEC
                							FROM (
											...
											SELECT 	R.ID_JUEGO ID_JUEGO,
													...
													S.DESCRIPCION DESCRIPCION_SUCURSAL,
													CASE
							                            WHEN R.VENTA_EMPLEADO = 'S' THEN 'VENTA CONTADO CASA CENTRAL'
							                            WHEN R.VENTA_CONTADO = 'S' THEN 'VENTA CONTADO'
							                            ELSE 'VENTA AGENCIA'
							                        END AS MODALIDAD
											FROM 	KANBAN.T_CUPON_ANTICIPADO TC,
													KANBAN.T_REPARTO_INTELIGENTE R,
            ...
            $rs_billetes_kanban = sql_kanban($sql_migrar_anticipado, array($id_juego, $sorteo, $id_juego, $sorteo));
