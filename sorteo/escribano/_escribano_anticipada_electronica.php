    ...
    function buscar_ganador(data) {
        ...
        if (typeof data.datosIncentivo[0] != 'undefined') {
            ...
            $('#agencia').html((data.datosIncentivo[0].desc_agencia));*/
             ...
             if(data.datosIncentivo[0].id_agencia != null){

               $('#agencia').html(pad(data.datosIncentivo[0].id_agencia,4));
               $('#nombre_d').html(data.datosIncentivo[0].nombre);
            }
            else if(data.datosIncentivo[0].nombre == 'VENTA CONTADO CASA CENTRAL'){
               $('#agencia').html('');
               $('#nombre_d').html('VENTA CONTADO CASA CENTRAL');
            }
            else if(data.datosIncentivo[0].nombre == 'VENTA CONTADO'){
               $('#agencia').html('');
               $('#nombre_d').html('VENTA CONTADO '.data.datosIncentivo[0].desc_sucursal);
            }
...
/*        $('#premio_prox_sorteo_r').html(data.prox_sorteo);*/
        ...
        for (var i = 0; i < data.ganadores.length; i++) {
            if(data.ganadores[i].AGENCIA != null)
                $('#ganadores').append('<div class="ganador"><div class="billete">'+pad(data.ganadores[i].BILLETE,5)+'</div><div class="fraccion">'+pad(data.ganadores[i].FRACCION,2)+'</div><div class="premio_r">'+data.ganadores[i].PREMIO+'</div><div class="agencia">'+pad(data.ganadores[i].AGENCIA,4)+'</div><div class="sucursal">'+data.ganadores[i].SUCURSAL+'</div></div>');
            else if(data.ganadores[i].NOMBRE == 'VENTA CONTADO CASA CENTRAL')
               $('#ganadores').append('<div class="ganador"><div class="billete">'+pad(data.ganadores[i].BILLETE,5)+'</div><div class="fraccion">'+pad(data.ganadores[i].FRACCION,2)+'</div><div class="premio_r">'+data.ganadores[i].PREMIO+'</div><div class="agencia">VENTA CONTADO</div><div class="sucursal">CASA CENTRAL</div></div>');
            else if(data.ganadores[i].NOMBRE == 'VENTA CONTADO')
               $('#ganadores').append('<div class="ganador"><div class="billete">'+pad(data.ganadores[i].BILLETE,5)+'</div><div class="fraccion">'+pad(data.ganadores[i].FRACCION,2)+'</div><div class="premio_r">'+data.ganadores[i].PREMIO+'</div><div class="agencia">'+data.ganadores[i].NOMBRE+'</div><div class="sucursal">'+data.ganadores[i].SUCURSAL+'</div></div>');
        }
