	...
	<script type="text/javascript">
		...
		function animarSorteo(data){
			...
			for(var i=0;i<data.billetesZona1.length;i++){
				...
				$('#contenido').append('<span id="bola'+data.billetesZona1[i].posicion+'">'+data.billetesZona1[i].numero+'</span>');
				...
				if(data.billetesZona1[i].posicion == 1){
					...
					if(data.billetesZona1[i].vendido == 'SI')
						 $('#primer_premio_resumen').css('background-image', 'url(escribano_img/primer_premio_4813.png)');
					else if(data.billetesZona1[i].vendido == 'NO')
						$('#primer_premio_resumen').css('background-image', 'url(escribano_img/no_vendido_4813.png)');
					*/
			...
			for(var i=0;i<data.billetesZona2.length;i++){
				$('#contenido').append('<span id="bola'+data.billetesZona2[i].posicion+'">'+data.billetesZona2[i].numero+'</span>');
