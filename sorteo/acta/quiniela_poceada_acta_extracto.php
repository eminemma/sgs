...
$pdf->Cell(6, 5, utf8_decode('ND'), 0, 0, 'C');
$pdf->Cell(6, 5, '', 0, 0, 'C');
...
while ($row_extracciones_comp = $rs_extracciones_comp->FetchNextObject($toupper = true)) {
    ...
    if (strpos($row_extracciones_comp->SORTEO_ASOC, 'QUINIELA DUPLICADO') !== false && $primer_corte == false) {
        $pdf->ln(5);
        $pdf->SetX(27);
        $pdf->Cell(6, 5, $row_extracciones_comp->POSICION_DUPLICADO, 1, 0, 'C');
        $pdf->Cell(6, 5, $row_extracciones_comp->NUMERO, 1, 0, 'C', 1);
        $pdf->Cell(6, 5, ':', 0, 0, 'C');
    } else {
...
while ($row = $rs_extracciones->FetchNextObject($toupper = true)) {
    ...
    $pdf->SetX($x);
    $pdf->SetFont('Times', '', 11);
    $pdf->Cell(20, 5, ucwords(strtolower('POSICION ' . $row->POSICION)), 1, 0, 1);
