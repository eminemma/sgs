...
try {
    $rs_extracciones = sql("   SELECT decode(tpp.tipo_premio,'ESPECIE',tde.descripcion_especia,'EFECTIVO',tpp.premio_efectivo)
        ...
        ORDER BY te.zona_juego desc ,te.posicion asc, te.ORDEN DESC", array($_SESSION['sorteo'], $_SESSION['id_juego']));
} catch (exception $e) {die($db->ErrorMsg());}
...
$pdf->Cell(75, 5, 'PREMIO', 1, 0, 'C');
$pdf->Cell(80, 5, 'POSICION', 1, 0, 'C');
$pdf->Cell(20, 5, 'BILLETE', 1, 0, 'C');
...
while ($row_extraccion = $rs_extracciones->FetchNextObject($toupper = true)) {
    $pdf->Cell(75, 5, $row_extraccion->DESCRIPCION_ESPECIA, 'B', 0, 'L');
    $pdf->Cell(80, 5, $row_extraccion->DESCRIPCION, 'B', 0, 'L');
    $pdf->Cell(20, 5, str_pad($row_extraccion->NUMERO, 5, 0, STR_PAD_LEFT), 'B', 0, 'C');
    $pdf->Cell(20, 5, str_pad($row_extraccion->FRACCION, 2, 0, STR_PAD_LEFT), 'B', 1, 'C');
    ...
    try {
        $rs_dist = sql(" SELECT DESCRIPCION_AGENCIA,LOCALIDAD,ID_AGENCIA,PROVINCIA,DESCRIPCION_SUCURSAL,ID_SUCURSAL,PROVINCIA
	              ...
	              AND BILLETE  = ?
				  AND FRACCION = ?
	              group by DESCRIPCION_AGENCIA,LOCALIDAD,ID_AGENCIA,PROVINCIA,DESCRIPCION_SUCURSAL,ID_SUCURSAL,PROVINCIA", array($_SESSION['sorteo'], $_SESSION['id_juego'], $row_extraccion->NUMERO, $row_extraccion->FRACCION));
    } catch (exception $e) {die($db->ErrorMsg());}
    ...
    while ($row_dist = siguiente($rs_dist)) {
        ...
        $pdf->MultiCell(195, 10, 'Vendido en: ' . $distribuido, 1, 'C');
    }
