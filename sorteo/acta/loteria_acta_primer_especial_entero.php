...
$rs_extracciones->MoveFirst();
$row_extraccion = $rs_extracciones->FetchNextObject($toupper = true);
...
try {
    $rs_fracciones = sql(" SELECT LPAD(FRACCION,2,0) AS FRACCION,DESCRIPCION_AGENCIA,LOCALIDAD,DESCRIPCION_SUCURSAL,PROVINCIA,ID_SUCURSAL,ID_AGENCIA
              ...
              WHERE SORTEO = ?
              AND ID_JUEGO = ?
              AND BILLETE  = ?", array($_SESSION['sorteo'], $_SESSION['id_juego'], $row_extraccion->NUMERO));
} catch (exception $e) {die($db->ErrorMsg());}
