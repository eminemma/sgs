...
function guardar_extraccion(nombre_juego){
  var afecta=buscar_valor_por_campo('posicion',$('#posicion').val(),'afecta');
   $.post(
    'sorteo/operador/quiniela/quiniela_sorteador_ajax.php',
    { juego:nombre_juego,
    ...
    function(data){
      ...
      if(data.tipo=='success'){
        ...
        if(loteria_tradicional.cantidad_premios_tradicional==1)
          mostrarMensaje({mensaje:'Ultima Extraccion del Juego Quiniela',tipo:'info'});
        if(loteria_tradicional.cantidad_premios_extraordinario==1)
          mostrarMensaje({mensaje:'Ultima Extraccion del Premios Quiniela',tipo:'info'});
      }
  ...
  $.post(
    ...
    function(data){      
      try{
          ...
          console.log(loteria_tradicional);
