  ...
  $fecha_prescripcion=isset($_POST['fecha_prescripcion']) ? $_POST['fecha_prescripcion'] : '';
   ...
   if($jefe_sorteo=='-1'){
    ...
    try{
      ...
      $ok=sql("UPDATE sgs.T_INCENTIVO_SORTEO
              SET id_escribano=?,id_operador=?,id_jefe=?,fecha_sorteo=to_date(?,'dd/mm/yyyy'),fecha_hasta_pago_premios=to_date(?,'dd/mm/yyyy')
             WHERE ID_INCENTIVO_SORTEO             = ?", array($escribano,$operador,$jefe_sorteo,$fecha_sorteo,$fecha_prescripcion,$id));
